...
const showPopUp = (message, isError, isRedirect=null) => {
    ...
    if (isError) {
        ...
        element = document.getElementById("success-msg");
    }
    ...
    timeoutId = setTimeout(() => {
        ...
        if (isRedirect != null) window.location.href = isRedirect;
    }, 2000);
};
...
const renderUser = async (isBio=false) => {
    if (!localUser && !localUser?.username) {
        ...
        if (data.success) {
            ...
            localUser = await JSON.parse(localStorage.getItem("user"));
        }
        else {
            return;
        }
...
function renderData(user, isBio) {
    ...
    const loginShow = document.getElementsByClassName("loggedin");
    const logoutShow = document.getElementsByClassName("logout");
    const userUsername = document.getElementsByClassName("user-name");
    const userName = document.getElementsByClassName("user-username");
    const userAvatar = document.getElementsByClassName("user-avatar");

    for (let el of loginShow) {
        el.style.display = "block";
    }

    for (let el of logoutShow) {
